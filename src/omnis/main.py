...
def find_config_file(explicit_path: Path | None = None) -> Path:
    ...
    candidates = [
        ...
        Path("config/chomiamos.yaml"),
        Path("config/examples/glfos.yaml"),
    ]
    here = Path(__file__).resolve()
    for base in list(here.parents)[:6]:
        candidates.append(base / "share" / "omnis" / "config" / "chomiamos.yaml")
        candidates.append(base / "share" / "omnis" / "omnis.yaml")
        candidates.append(base / "config" / "chomiamos.yaml")
